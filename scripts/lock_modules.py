...
def get_commit_info():
  ...
  return {"sha": commit, "date": date.replace("-", "")}
...
def save_module_registry(registry):
  ...
  for module_id, versions in registry.items():
    for version in versions:
      ...
      mapping[registry_id] = {"commit": version["commit"], "path": version["path"]}
...
def main():
  ...
  parser.add_argument(
    "-d", "--dirty", action="store_true", help="allow dirty working dir"
  )
  parser.add_argument(
    "-v",
    "--verify",
    action="store_true",
    help="verify %s is up to date" % module_registry_file,
  )
